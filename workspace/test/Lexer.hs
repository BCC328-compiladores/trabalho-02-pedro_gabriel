...
testKeywords = TestCase $ do
    let input = "struct forall func let return print scan if elif else while for new int float string bool void"
    let expected = [TkStruct, TkForAll, TkFunc, TkLet, TkReturn, TkPrint, TkScan, TkIF, TkElif, TkElse, TkWhile, TkFor, TkNew, TkInt, TkFloat , TkString  , TkBool, TkVoid]
